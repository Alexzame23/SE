...
int main() {
	...
	usleep(1000); // Esperar 100 ms antes de la siguiente lectura
	//    }

	u32 adc_val;         // Valor leído del potenciómetro
	u32 aux = 1;
	u32 persistence;
	u32 data_out;       // Valor de persistencia calculado
	u32 cmd;

	// Montamos comando: canal en bits 24–26
	cmd = aux << (31-26);

	while (1) {
		// Lee el valor del potenciómetro
		CONVERSORAD_mWriteReg(XPAR_CONVERSORAD_0_S00_AXI_BASEADDR, CONVERSORAD_S00_AXI_SLV_REG0_OFFSET, cmd);

	    do {
	        data_out = CONVERSORAD_mReadReg(XPAR_CONVERSORAD_0_S00_AXI_BASEADDR, CONVERSORAD_S00_AXI_SLV_REG0_OFFSET);
	    } while ((data_out & (1 << 23)) == 0);
		//do {
		adc_val = CONVERSORAD_mReadReg(XPAR_CONVERSORAD_0_S00_AXI_BASEADDR, CONVERSORAD_S00_AXI_SLV_REG1_OFFSET);
		...
		persistence = (adc_val * 1000) / 255; // Normaliza entre 0 y 1000 ms
